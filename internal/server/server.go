...
import (
	...
	"github.com/huybui/groxpi/internal/cache"
	"github.com/huybui/groxpi/internal/config"
	"github.com/huybui/groxpi/internal/pypi"
	"github.com/huybui/groxpi/internal/storage"
)
...
func (s *Server) handleListPackages(c *fiber.Ctx) error {
	...
	if len(packages) == 0 {
		// Fetch from PyPI
		var err error
		packages, err = s.pypiClient.GetPackageList()
		if err != nil {
	...
	if wantsJSON(c) {
		...
		jsonData, err := sonic.ConfigFastest.Marshal(response)
		if err != nil {
			return err
		}
		...
		cacheKey := "json:package-list"
		s.responseCache.Set(cacheKey, jsonData, s.config.IndexTTL)
